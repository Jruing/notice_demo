...
func UserAdd(c *gin.Context) {
	...
	if err != nil {
		logger.Error("用户新增失败", zap.String("uuid", log_uuid.String()), zap.String("error", err.Error()))
		c.JSON(200, gin.H{
			"Code": 0,
			"Msg":  "用户新增失败",
		})
	} else {
		logger.Info("用户新增成功", zap.String("uuid", log_uuid.String()))
		c.JSON(200, gin.H{
			"Code": 1,
			"Msg":  "用户新增成功",
		})
...
func UserDelete(c *gin.Context) {
	...
	if err != nil {
		logger.Error("用户删除失败", zap.String("uuid", log_uuid.String()), zap.String("error", err.Error()))
		c.JSON(200, gin.H{
			"Code": 0,
			"Msg":  "用户删除失败",
		})
	} else {
		logger.Info("用户删除成功", zap.String("uuid", log_uuid.String()))
		c.JSON(200, gin.H{
			"Code": 1,
			"Msg":  "用户删除成功",
		})
